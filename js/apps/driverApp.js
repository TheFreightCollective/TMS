...
function updateDriverHeader(){ const info = el('sessionInfo'); const loginSection = el('loginSection'); if(!state.currentUser){ if(info) info.textContent = 'Not logged in'; el('logoutBtn')?.classList.add('hidden'); el('driverPanel')?.classList.add('hidden'); el('changePasswordPanel')?.classList.add('hidden'); if(loginSection) loginSection.style.display = 'block'; return; } el('logoutBtn')?.classList.remove('hidden'); if(info) info.textContent = state.currentUser.email || state.currentUser.id; if(loginSection) loginSection.style.display = 'none'; }
async function bootstrapDriverUser(user){ state.currentUser = user; state.currentProfile = null; state.currentDriver = null; if(!user){ hasBootstrappedOnce = false; updateDriverHeader(); renderDriverJobs([]); return; } const { data, error } = await sb.from('profiles').select('*').eq('id', user.id).maybeSingle(); if(error) console.error('Profile load error:', error); state.currentProfile = data || { role: roleFromEmail(user.email) || null }; hasBootstrappedOnce = true; updateDriverHeader(); if(state.currentProfile.role !== 'driver'){ toast('This page is for drivers only.', true); el('driverPanel')?.classList.add('hidden'); el('changePasswordPanel')?.classList.add('hidden'); renderDriverJobs([]); return; } el('driverPanel')?.classList.remove('hidden'); el('changePasswordPanel')?.classList.remove('hidden'); await loadDriverContext(); await loadDriverJobs(); }
...
function bindDriverAuthEvents(){ el('loginBtn')?.addEventListener('click', async evt => { evt.preventDefault(); await login(); }); el('logoutBtn')?.addEventListener('click', logout); el('checkSessionBtn')?.addEventListener('click', refreshFromAuth); sb.auth.onAuthStateChange(function(event, session){ if(event === 'INITIAL_SESSION') return; if(event === 'SIGNED_IN' && session?.user){ if(hasBootstrappedOnce && session.user.id === state.currentUser?.id) return; setTimeout(() => bootstrapDriverUser(session.user), 0); } if(event === 'SIGNED_OUT'){ hasBootstrappedOnce = false; setTimeout(() => bootstrapDriverUser(null), 0); } }); }
function initDriverApp(){ bindDriverAuthEvents(); bindDriverJobEvents(); bindDriverProofEvents(); bindChangePasswordEvents(); refreshFromAuth(); }
initDriverApp();
